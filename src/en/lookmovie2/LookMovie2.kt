package eu.kanade.tachiyomi.animeextension.en.lookmovie2
...
class LookMovie2 : ParsedHttpSource() {

    override val name = "LookMovie2"
    ...
    override fun popularAnimeSelector(): String = "div.mv-item, div.movie-item, article.item"
    override fun popularAnimeFromElement(element: Element): SAnime = SAnime.create().apply {
        setUrlWithoutBaseUrl(element.select("a").attr("href"))
        title = element.select("h2, .title, a").text().trim()
    ...
    override fun animeDetailsParse(document: Document): SAnime = SAnime.create().apply {
        ...
        genre = document.select("div.genre a, div.row-line:contains(Genre) a").joinToString(", ") { it.text() }
        description = document.select("div.description, div.detail-desc, p.story").text().trim()
        thumbnail_url = document.select("div.poster img, div.film-poster img").attr("abs:src")
    }

    // Episodes / Streams List
    override fun episodeListSelector(): String = "div.episodes-ul a, ul.episodes li a, div.episode-item a"
    ...
    override fun videoFromElement(element: Element): Video {
        ...
        return Video(streamUrl, "LookMovie2 Server", streamUrl)
    }
